...
## 3. Census: Download county-level data for county-level opportunity structure indicators
#     - race, median income, poverty, IPR, ed attain, industry, unemployment
#     --- Use tigris, include geometry
###           CHALLENGE: # cases incr exponentially when use st_join
#     --- Use tigris, exlude geometry
###           CHALLENGE: geometry drops when join sf/df using left_join 
## 4. NCES: Download NCES CCD files
## 5. Census: Initial cleaning - Census files
#     - rename vars, e.g., 02001_002 -> White, combine categories, pivot wide
#          drop outlying areas, run basic descriptives
## 6. Census: Drop outlying areas in sf, merge Census dataframes
#     - if didn't download indicator data as shapefiles
...
#     --- CHALLENGE: ~316 files no lat/long - in Census but not CCD (2009BY)
## 9. COMBINED: Check all(st_is_valid), st_is_empty(), bbox - predicates?
...
```{r}
#| label: Download NCES CCD Directory files, incl GEOIDS - save to load into VCE
...
```{r}
#| label: Drop outlying areas in sf, merge Census dataframes
